describe('Book Favorites App', () => {
  ...
  it('should allow saving a comment for a favorite', () => {
    const commentText = 'My top pick this month';
    ...
    cy.get('input[id^="favorite-comment-"]').first().clear().type(commentText);
    cy.contains('button', 'Save Comment').first().click();

    cy.reload();
    cy.get('input[id^="favorite-comment-"]').first().should('have.value', commentText);
  });
});
